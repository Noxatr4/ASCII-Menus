...
class Menu:
    ...
    def __init__(self, title_menu: str, option_per_column: int,
        ...
        def set_scroll_bar():
            ...
            for i, y in enumerate(scrollbar_groups):
                for x in y:
                    scroll_bar = DEFAULT_CHARACTERS["ScrollBarValues"][True]
    ...
    def _check_width_options(self, str_option):
    ...
    def _options_list_processing(self, input_options_list: list[str]):
        ...
        def same_len_list_elements(input_list: list[str]):
            ...
            adjusted_elements = input_options_list + [""] * (self._max_index - list_size)
        ...
        def add_cursor_to_option(input_list: list[str]):
            ...
            options_with_cursor: list[list[str] | str] = list(
                map(lambda opt: [cursor, opt], body_menu)
            )
    ...
    def show_frame_menu(self):
        coord = self._cursor_coordinates
        page = self._cursor_coordinates[0] + 3
        row = self._cursor_coordinates[1]
        col = self._cursor_coordinates[2]
...
class Menu2:
    ...
    def __init__(self, title_menu: str, option_per_column: int,
        ...
        def _options_list_processing(input_options_list: list[str]):
            ...
            limit_row_menu = (DEFAULT_CHARACTERS["MenuCorner"]
                              + DEFAULT_CHARACTERS["MenuLimitRowBody"] * self._characters_per_row
                              + DEFAULT_CHARACTERS["MenuCorner"])

            title_row = (DEFAULT_CHARACTERS["RowLimit"]
                         +self._title_menu.center(self._characters_per_row)
                         +DEFAULT_CHARACTERS["RowLimit"])

            separator_row = (DEFAULT_CHARACTERS["RowLimit"]
                            + DEFAULT_CHARACTERS["MenuLimitRowBody"] * self._characters_per_row
                            + DEFAULT_CHARACTERS["RowLimit"])

            # Create title
            title_list = [limit_row_menu, title_row, separator_row]
            del title_row, separator_row
            ...
            adjust_list_options = list(
                map(check_width_options, adjust_list_options)
            )

            # Create columns; Add cursor to options
            cursor = DEFAULT_CHARACTERS["cursor_values"][False]
            options_list_processed: list[list[str] | str] = list(
                map(lambda opt : [cursor, opt], adjust_list_options)
            )
            del cursor, adjust_list_options


            # Structural change of list to approach the coord system.
            # Create rows
            rows_list = []
            scroll_bar = DEFAULT_CHARACTERS["ScrollBarValues"][False]
            row_limit = DEFAULT_CHARACTERS["RowLimit"]
            empty_body_row = DEFAULT_CHARACTERS["BodyEmptyRow"] * (self._characters_per_row - 1)
            empty_row = [row_limit, empty_body_row, scroll_bar, row_limit]

            for i in range(0, len(options_list_processed), self._option_per_column):
                # Add Row Options
                row = options_list_processed[i : i + self._option_per_column]
                # Add BorderRow
                row.insert(0, row_limit)
                # Add ScrollBar
                row.append(scroll_bar)
                # Add BorderRow
                row.append(row_limit)
                # Add Row to list
                rows_list.append(row.copy())
            del options_list_processed, list_size, scroll_bar, row_limit, empty_body_row, row


            # Create Pages
            create_pages = []
            for i in range(0, len(rows_list), self._options_rows_per_page):
                page_options = rows_list[i : i + self._options_rows_per_page]
                page_menu = []
                [page_menu.extend((empty_row.copy(), options_row)) for options_row in page_options]
                page_menu.append(empty_row.copy())

                create_pages.append(page_menu)

            default_menu = title_list + create_pages + [limit_row_menu]
            return default_menu
