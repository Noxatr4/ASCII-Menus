...
class Menu:
    ...
    def __init__(self, title_menu: str, option_per_column: int,
        ...
        """
        def options_list_processing(input_options_list: list[str]):
            """
            ...
            # Create Pages
            create_pages = []
            for i in range(0, len(options_list_processed), self._rows_per_page):
                create_pages.append(options_list_processed[i : i + self._rows_per_page])
        ...
        self._options_list: list = options_list_processing(options_list)
    ...
    def menu_crafter(self, mode: str = "solo"):
        ...
        def mode_menu_generate(input_row: str | tuple[str, ...]):
            if mode == "solo":
                print(input_row)
            elif mode == "display":
        ...
        def set_scrollbar():
            scrollbar_groups = []
            rows_group = int(self._rows_per_page * 2 / self._number_pages)
            module = self._rows_per_page * 2 % self._number_pages
        ...
        def create_row(type_row: str, index_options_row: int = 0):
            match type_row:
                case "limit":
        ...
        for n in range(steps_to_create_menu):
            if n == 1:
                ...
                mode_menu_generate(title)
            elif n == 2:
                separator_row = create_row("separator")
                mode_menu_generate(separator_row)
            elif n == 3:
                active_scrollbar = set_scrollbar()[coord_page]
                for c in range(self._rows_per_page * 2):
                    options_row = create_row("options", index_options_row=c)
                    mode_menu_generate(options_row)

            else:
                limit_row = create_row("limit")
                mode_menu_generate(limit_row)
